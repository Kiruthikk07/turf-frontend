      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
            ...
            <a href="/bookings" className="text-xs text-green-400 hover:text-green-300 flex items-center gap-1">
            ...
            {recentBookings.map((b) => (
              <div key={b.id} className="flex items-center justify-between px-5 py-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-[10px] font-black text-white flex-shrink-0">
                    {b.player.split(" ").map(n => n[0]).join("")}
                  </div>
                  <div>
                    <p className="text-xs font-medium text-white/80">{b.player}</p>
                    <p className="text-[10px] text-white/35">{b.turf} · {b.slot}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs font-semibold text-white/70">{b.amount}</span>
        ...
        >
          <div className="flex items-center gap-2 px-5 py-4 border-b border-white/5">
            ...
            <h2 className="text-sm font-semibold">Today's Slots</h2>
          </div>
          <div className="p-4 space-y-2">
            {upcomingSlots.map((slot) => (
              <div key={slot.time} className="flex items-center justify-between">
                <span className="text-xs text-white/40 w-12">{slot.time}</span>
                <div className="flex-1 mx-3">
                  <div className={`h-6 rounded-lg text-[10px] flex items-center px-2 font-medium ${
                    slot.status === "booked"
