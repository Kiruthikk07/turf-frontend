    ...
    <div className="space-y-6 w-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-base mt-1">{bookings.length} total bookings</p>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {["all", "confirmed", "pending", "cancelled"].map((s) => (
            <button
              ...
              className={`text-sm px-4 py-2.5 rounded-xl font-medium capitalize transition-all ${statusFilter === s ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-white/5 border border-white/8 text-white/40 hover:text-white"}`}
            >
              {s}
            </button>
          ))}
      ...
      <div className="bg-white/3 border border-white/6 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/5 text-white/35 text-sm">
                ...
                <th className="text-left px-5 py-3.5 font-medium">Slot</th>
            ...
            <tbody className="divide-y divide-white/4">
              {filtered.map((b, i) => (
                ...
                >
                  <td className="px-5 py-3.5 text-sm text-green-400 font-mono">{b.id}</td>
                  <td className="px-5 py-3.5">
                    <p className="text-sm font-medium text-white/80">{b.player}</p>
                    <p className="text-xs text-white/30">{b.mobile}</p>
                  </td>
                  <td className="px-5 py-3.5">
                    <p className="text-sm text-white/70">{b.turf}</p>
                    <p className="text-xs text-white/30">{b.venue}</p>
                  </td>
                  <td className="px-5 py-3.5 text-sm text-white/50">{b.slot}</td>
                  <td className="px-5 py-3.5 text-sm text-white/50">{b.date}</td>
                  <td className="px-5 py-3.5 text-sm font-semibold text-white/80">₹{b.amount}</td>
                  <td className="px-5 py-3.5">
                    <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${statusStyle[b.status]}`}>{b.status}</span>
                  </td>
                  <td className="px-5 py-3.5">
                    <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${paymentStyle[b.payment]}`}>{b.payment}</span>
                  </td>
                </motion.tr>
              ))}
            </tbody>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-14 text-white/30 text-base">No bookings found</div>
          )}
        </div>
      </div>
    </div>
