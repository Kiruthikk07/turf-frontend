    ...
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-white/40 text-sm mt-0.5">Track all transactions and revenue</p>
        </div>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {summaryCards.map((c, i) => (
          <motion.div
            ...
            className={`bg-white/3 border ${c.border} rounded-2xl p-4`}
          >
            <p className="text-[10px] text-white/35 mb-2">{c.label}</p>
      ...
      <div className="bg-white/3 border border-white/6 rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
              ...
              {filtered.map((p, i) => (
                ...
                >
                  <td className="px-5 py-3">
                    ...
                    <p className="text-[10px] text-white/25">{p.bookingId}</p>
                  </td>
                  <td className="px-5 py-3 text-xs text-white/70">{p.player}</td>
                  <td className="px-5 py-3 text-xs text-white/50">{p.turf}</td>
                  <td className="px-5 py-3 text-xs font-bold text-white/80">₹{p.amount}</td>
                  <td className="px-5 py-3 text-xs text-white/50">{p.method}</td>
                  <td className="px-5 py-3">
                    <p className="text-xs text-white/60">{p.date}</p>
                    <p className="text-[10px] text-white/30">{p.time}</p>
                  </td>
                  <td className="px-5 py-3">
          ...
          {filtered.length === 0 && (
            <div className="text-center py-12 text-white/30 text-sm">No payments found</div>
          )}
        </div>
      </div>
    </div>
